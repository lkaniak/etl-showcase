...
from my_marketplace_etl.adapters.postgres.custom_update.ignore_columns_to_update_on_upsert import (
    IgnoreColumnsToUpdateOnUpsert,
)
...
def _date_column_names(model: type[BaseEtlModel]) -> set[str]:
    return {
        column.name
...
def _coerce_date_column_value(value):
    if value is None:
        return None
...
def _coerce_records_for_postgres(model: type[BaseEtlModel], records: list[dict]) -> list[dict]:
    date_columns = _date_column_names(model)
    if not date_columns:
        return records
    coerced = []
    for record in records:
        ...
        for column_name in date_columns:
            if column_name in row:
                row[column_name] = _coerce_date_column_value(row[column_name])
        coerced.append(row)
    return coerced


class PostgresRecordWriter:
    def __init__(self, connection_pool: asyncpg.Pool, config: BaseSettings):
        self.connection_pool = connection_pool
        self.app_config = config
        self.custom_update_routines = {
            repr(IgnoreColumnsToUpdateOnUpsert()): IgnoreColumnsToUpdateOnUpsert.impl,
        }
    ...
    async def upsert_records(self, model: type[BaseEtlModel], records: list[dict]) -> None:
        if not records:
            return
        records = _coerce_records_for_postgres(model, records)
